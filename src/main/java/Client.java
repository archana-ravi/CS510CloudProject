...
public class Client
{
	...
	public static void query1() {
		...
		while (resultSet.iterator().hasNext()) {
			...
			System.out.println("\tNumber of speeds > 100 = " + count + "\n");
		}
	...
	public static void query3() {
        ...
        //System.out.println("Length of Foster NB station: " + length);

        String cqlStatement1 = "SELECT sum(speed) as sum FROM cs510.freeway_data WHERE locationtext = 'Foster NB' AND starttime >= '2011-09-21 07:00:00+0000' AND starttime <='2011-09-21 09:00:00+0000' ALLOW FILTERING;";
        resultSet = session.execute(cqlStatement1);
        row = resultSet.one();
        int sumSpeed7To9AM = row.getInt("sum");
        cqlStatement1 = "SELECT count(speed) as count FROM cs510.freeway_data WHERE locationtext = 'Foster NB' AND starttime >= '2011-09-21 07:00:00+0000' AND starttime <='2011-09-21 09:00:00+0000' ALLOW FILTERING;";
        resultSet = session.execute(cqlStatement1);
        row = resultSet.one();
        long countSpeed7To9AM = row.getLong("count");
        System.out.println("\tAvg travel time for 7-9 am is: " + ((float)sumSpeed7To9AM)/countSpeed7To9AM * 3600 + " seconds");

        String cqlStatement2 = "SELECT sum(speed) as sum FROM cs510.freeway_data WHERE locationtext = 'Foster NB' AND starttime >= '2011-09-21 16:00:00+0000' AND starttime <'2011-09-21 18:00:00+0000' ALLOW FILTERING;";
        resultSet = session.execute(cqlStatement2);
        row = resultSet.one();
        int sumSpeed4To6PM = row.getInt("sum");
        cqlStatement2 = "SELECT count(speed) as count FROM cs510.freeway_data WHERE locationtext = 'Foster NB' AND starttime >= '2011-09-21 16:00:00+0000' AND starttime <'2011-09-21 18:00:00+0000' ALLOW FILTERING;";
        resultSet = session.execute(cqlStatement2);
        row = resultSet.one();
        long countSpeed4To6PM = row.getLong("count");
        System.out.println("\tAvg travel time for 4-5 pm is: " + ((float) sumSpeed4To6PM)/countSpeed4To6PM * 3600 + " seconds\n");
    }
	...
	public static void query5() {
        ...
        //Updating length of station 1140 back to original value of 2.14
